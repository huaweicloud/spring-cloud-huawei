...
public class ConfigServiceManagerUtils {
  ...
  public static void shutDownConfigService(NacosConfigServiceManager configServiceManager) {
    try {
      ...
      // if using import type load config, need to extras shut down bind task.
//      NacosConfigServiceManager importConfigService = chooseImportConfigServiceManager(configServiceManager);
//      if (importConfigService != null) {
//        importConfigService.getConfigService().shutDown();
//        LOGGER.warn("nacos config server shutDown success, serverAddr=[{}]", importConfigService.getServerAddr());
//      }
