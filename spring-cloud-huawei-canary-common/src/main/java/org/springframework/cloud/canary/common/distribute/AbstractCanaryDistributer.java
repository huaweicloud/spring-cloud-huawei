...
import org.springframework.cloud.canary.common.model.TagItem;
import org.springframework.cloud.canary.common.util.VersionCompareUtil;
...
public class AbstractCanaryDistributer<T extends Server, E> implements CanaryDistributer<T, E> {
    ...
    public List<T> getFiltedServer(Map<TagItem, List<T>> allServer,
        ...
        return allServer
                .get(CanaryRuleCache.getServiceInfoCacheMap().get(targetServiceName).getNextInvokeVersion(rule));
    }

    /**
     * 1.过滤targetService
     * 2.返回按照version和tags分配list
     *
     * @param serviceName
     * @param list
     * @return
    ...
    private Map<TagItem, List<T>> getDistributList(String serviceName,
        ...
        String latestV = CanaryRuleCache.getServiceInfoCacheMap().get(serviceName).getLatestVersionTag().getVersion();
        ...
        for (T server : list) {
            ...
            if (getServerName.apply(ms).equals(serviceName)) {
                ...
                if (invokeRule.isWeightLess() && getVersion.apply(ms).equals(latestV)) {
                    versionServerMap.get(invokeRule.getRoute().get(invokeRule.getRoute().size() - 1).getTagitem()).add(server);
                }
    ...
    public void initLatestVersion(String serviceName, List<T> list) {
        ...
        for (T server : list) {
            E ms = getIns.apply(server);
            if (getServerName.apply(ms).equals(serviceName)) {
                if (latestVersion == null || VersionCompareUtil.compareVersion(latestVersion, getVersion.apply(ms)) == -1) {
    ...
    public List<T> getLatestVersionList(List<T> list, String targetServiceName) {
        String latestV = CanaryRuleCache.getServiceInfoCacheMap().get(targetServiceName).getLatestVersionTag().getVersion();
