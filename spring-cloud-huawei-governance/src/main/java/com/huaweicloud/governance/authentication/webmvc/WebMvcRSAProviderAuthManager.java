...
public class WebMvcRSAProviderAuthManager {
  ...
  public WebMvcRSAProviderAuthManager(List<AccessController> accessControllers, Environment environment,
    ...
    try {
      ...
      if (environment.getProperty(GovernanceConst.AUTH_TOKEN_CHECK_ENABLED, boolean.class, true)
        ...
        String headerTokenKey = environment.getProperty(GovernanceConst.AUTH_TOKEN_HEADER_KEY, String.class,
            "X-SM-Token");
        String requestHeadToken = request.getHeader(headerTokenKey);
        RsaAuthenticationToken rsaToken = RSATokenCheckUtils.checkTokenInfo(authenticationAdapter, requestHeadToken,
            InvocationContextHolder.getOrCreateInvocationContext());
        extractor = AuthRequestExtractorUtils.createWebMvcAuthRequestExtractor(request, rsaToken.getServiceId(),
            rsaToken.getInstanceId());
      } else {
        extractor = AuthRequestExtractorUtils.createWebMvcAuthRequestExtractor(request, "", "");
      }
